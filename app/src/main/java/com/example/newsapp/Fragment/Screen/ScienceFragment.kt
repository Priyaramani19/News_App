...
class ScienceFragment : Fragment() {
    ...
    lateinit var scienceFragmentBinding : FragmentScienceBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        scienceFragmentBinding = FragmentScienceBinding.inflate(inflater,container,false)

        getCountry()

        return scienceFragmentBinding.root
    }

    fun getCountry(){
        var apiInterface = APIClient().getRetrofoit().create(APIInterface::class.java)
        apiInterface.getCountry("in","science","532a522ab79744f7a07aec970a5bdeaf").enqueue(object :
            ...
            override fun onResponse(call: Call<CountryModel>, response: Response<CountryModel>) {
                ...
                scienceFragmentBinding.shimmerEffect.isVisible = false
    ...
    fun setUpRV(l1: List<com.example.newsapp.ModelClassAPI.ArticlesItem?>) {
        ...
        scienceFragmentBinding.rvView.adapter = adapter
        scienceFragmentBinding.rvView.layoutManager = layoutManager
    }
