...
class SportFragment : Fragment() {
    ...
    lateinit var sportFragmentBinding : FragmentSportBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        sportFragmentBinding = FragmentSportBinding.inflate(inflater,container,false)


        getCountry()


        return sportFragmentBinding.root
    }

    fun getCountry(){
        var apiInterface = APIClient().getRetrofoit().create(APIInterface::class.java)
        apiInterface.getCountry("in","sport","532a522ab79744f7a07aec970a5bdeaf").enqueue(object :
            ...
            override fun onResponse(call: Call<CountryModel>, response: Response<CountryModel>) {
                ...
                sportFragmentBinding.shimmerEffect.isVisible = false
    ...
    fun setUpRV(l1: List<com.example.newsapp.ModelClassAPI.ArticlesItem?>) {
        ...
        sportFragmentBinding.rvView.adapter = adapter
        sportFragmentBinding.rvView.layoutManager = layoutManager
    }
