...
class TechnologyFragment : Fragment() {
    ...
    lateinit var techFragmentBinding: FragmentTechnologyBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        techFragmentBinding = FragmentTechnologyBinding.inflate(inflater,container,false)


        getCountry()


        return techFragmentBinding.root
    }

    fun getCountry(){
        var apiInterface = APIClient().getRetrofoit().create(APIInterface::class.java)
        apiInterface.getCountry("in","technology","532a522ab79744f7a07aec970a5bdeaf").enqueue(object :
            ...
            override fun onResponse(call: Call<CountryModel>, response: Response<CountryModel>) {
                ...
                techFragmentBinding.shimmerEffect.isVisible = false
    ...
    fun setUpRV(l1: List<com.example.newsapp.ModelClassAPI.ArticlesItem?>) {
        ...
        techFragmentBinding.rvView.adapter = adapter
        techFragmentBinding.rvView.layoutManager = layoutManager
    }
